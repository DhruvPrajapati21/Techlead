...
class _ShortageOfProductState extends State<ShortageOfProduct> with TickerProviderStateMixin {
  ...
  Future<void> _submitForm() async {
    try {
      // Show customized loading spinner dialog before starting the submission process
      showDialog(
        context: context,
        barrierDismissible: false, // Prevent closing the dialog when tapping outside
        builder: (context) => AlertDialog(
          backgroundColor: Colors.transparent, // Remove background to focus on spinner
          title: Column(
          ...
          content: Container(), // Empty container to avoid unnecessary padding
        ),
      );
  ...
  Future<String> _loadExcelPreview(File file) async {
    try {
      ...
      // Reading the first sheet and first 5 rows
  ...
  Widget _buildFilePreview(File file, String fileType) {
    ...
    // ✅ Direct PDF Preview Approach
    else if (fileType == 'pdf') {
      return FutureBuilder<pdfx.PdfDocument>(
        future: pdfx.PdfDocument.openFile(file.path), // Correctly returning a Future
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          } else if (snapshot.hasError) {
  ...
  Widget _buildChooseFilesButton() {
    return ScaleTransition(
      ...
      child: GestureDetector(
        ...
        child: Center(
          child: Container(
            width: 200, // Set a fixed width for a smaller button
  ...
  Widget _buildSubmitButton() {
    return ScaleTransition(
      scale: _buttonScaleAnimation,
      child: GestureDetector(
        onTap: _isSubmitting ? null : _submitForm, // Disable button if submitting
        child: Center(
          child: Container(
            width: 200, // Set a fixed width for a smaller button
            padding: const EdgeInsets.symmetric(vertical: 12), // Reduced vertical padding
            decoration: BoxDecoration(
              color: _isSubmitting ? Colors.grey : Colors.green, // Disabled color during submission
              borderRadius: BorderRadius.circular(12),
            ),
            child: _isSubmitting
                ? Container() // Hide button content during submission
                : _isSuccess
                ? const Center(
              child: Text(
                'Submit', // Show submit button again after success
                style: TextStyle(
                  color: Colors.white,
                  fontSize: 16,
                  fontWeight: FontWeight.bold,
            ...
            ) // Show button after success (just text or other design)
                : const Center(
              child: Text(
            ...
            children: [
              AnimatedBuilder(
                ...
                builder: (context, child) {
                  return FaIcon(
                    FontAwesomeIcons.boxOpen, // FontAwesome icon for product (open box)
                    color: _iconColorAnimation.value, // Animated color
